...
var time_became_free := Time.get_ticks_msec()

func _enter_tree() -> void:
	player_detection_area.body_entered.connect(on_player_enter.bind())
	time_became_free = Time.get_ticks_msec()
...
func _process(delta: float) -> void:
	player_detection_area.monitoring = (Time.get_ticks_msec() - time_became_free > state_data.intangibility_duration)
