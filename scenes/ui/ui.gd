...
func update_score() -> void:
	score_label.text = ScoreHelper.get_score_text(GameManager.current_matchup)

func update_flags() -> void:
	var countries := [GameManager.current_matchup.country_home, GameManager.current_matchup.country_away]
...
func on_score_changed() -> void:
	if not GameManager.is_time_up():
		...
		score_info_label.text = ScoreHelper.get_current_score_info(GameManager.current_matchup)
		animation_player.play("goal_appear")
	update_score()

func on_team_reset() -> void:
	if GameManager.current_matchup.has_someone_scored():
		animation_player.play("goal_hide")

func on_game_over(_country_winnger: String) -> void:
	score_info_label.text = ScoreHelper.get_final_score_info(GameManager.current_matchup)
