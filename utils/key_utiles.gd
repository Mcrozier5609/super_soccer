class_name KeyUtiles

enum Action {LEFT, RIGHT, UP, DOWN, SHOOT, PASS, UNLOCK}
...
static func is_action_press(scheme: Player.ControlScheme, action: Action) -> bool:
	return Input.is_action_pressed(ACTION_MAP[scheme][action])

static func is_action_just_press(scheme: Player.ControlScheme, action: Action) -> bool:
